...
class CertbotContainer {
  constructor(domain) {
    ...
    this.createCMD = ['certonly', '--webroot', '--non-interactive', '--webroot-path=/var/www/certbot', '--email', CERTBOT_EMAIL, '--agree-tos', '--no-eff-email', '-d', domain];
    this.renewCMD = ['renew', '--webroot', '--webroot-path=/var/www/certbot', '--cert-name', domain];
  }
